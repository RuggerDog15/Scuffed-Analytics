...
P_all = pd.DataFrame()
#load the models
lst = ["home_win", "home_loss", "home_draw"]
models = {}

for x in lst:
    model = xgboost.XGBClassifier()
    model.load_model(datafolder / f"model-{x}.json")
    models[x] = model
...
for ind in tqdm(fixt.index):
    ...
    for x in lst:  
        P_hat[x] = [p[1] for p in models[x].predict_proba(GameX)] 
...
for ind in tqdm(fixt.index):
    predictions.loc[ind, "Sum"] = predictions.loc[ind, "home_win"].item() + predictions.loc[ind, "home_loss"].item() + predictions.loc[ind, "home_draw"].item()
    for l in lst:
        predictions.loc[ind, f"{l}_norm"] = predictions.loc[ind, l].item() / predictions.loc[ind, "Sum"].item()

predictions.to_csv(datafolder / "cnl-predictions.csv", index=False)
